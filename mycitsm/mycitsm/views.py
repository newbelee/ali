...
ldappath = "ldap://10.10.46.71"#ldap服务器地址
...
def _validateLDAPUser(user):
    try:
        ...
        if(not len(result_data) == 0):
            print result_data[0][0], result_data,"xxxxxxxxxxxxxxxxxxxxxxxxx"
...
def LDAPLogin(userName,Password):
    try:
        if(Password==""):
            ...
            return
        dn,userinfo = GetDn(userName,10)
        print dn,userinfo,"xxxxxxxx1111"
        if(dn==''):
...
def adddbserverinfo(request):
    ...
    if request.method=='POST':
        ...
        backupip=request.POST.get('backupip','')
        dns=request.POST.get('dns','')
        appname=request.POST.get('appname','')
        vip=request.POST.get('vip','')
        if not hostname:
            errors.append('必须提供主机名。')
        if not ip:
            errors.append('必须提供ip地址。')
        if errors:
            return render_to_response('error.html',{
                'errors':errors,
                'user':request.user,
...
values(%s,%s,%s,%s,%s,%s,%s,%s,%s);""",[dbtype,env,appname,vip,hostname,ip,backupip,dns,port])
        cursor.connection.commit()
        cursor.close()
        return HttpResponseRedirect("/serveradmin/dbserverinfo/")        
